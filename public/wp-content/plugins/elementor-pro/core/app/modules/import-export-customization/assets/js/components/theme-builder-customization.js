...
export function ThemeBuilderCustomization({
  state,
  settingKey,
  onStateChange,
  data,
  ...
  const loadConflicts = async () => {
    ...
    try {
      ...
      const formattedConflicts = actualConflicts.map(
        ([importedTemplateId, conflictsList]) => {
  ...
  return (
    ...
    >
      ...
      {state?.enabled && isImport && 0 < conflicts.length && (
        <Box sx={{ mt: 1 }}>{renderConflictTable()}</Box>
      )}
    </Box>
  );
}
