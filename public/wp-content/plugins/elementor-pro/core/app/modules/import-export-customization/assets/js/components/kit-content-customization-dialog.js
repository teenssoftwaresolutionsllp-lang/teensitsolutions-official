...
export function KitContentCustomizationDialog({
  ...
  } = useKitCustomizationPages({ open, data });
  const {
    isLoading: isTaxonomiesLoading,
    taxonomyOptions,
    isLoaded: isTaxonomiesLoaded,
  } = useKitCustomizationTaxonomies({ open, data });
      ...
      taxonomies:
        isTaxonomiesLoaded || isImport
          ? data.customization.content?.taxonomies ||
            taxonomyOptions.map(({ value }) => value)
    ...
    setSettings((prevSettings) => ({
      ...
      customPostTypes: customPostTypes
        ? data.customization.content?.customPostTypes ||
          customPostTypes.map(({ value }) => value)
