...
export default function useTemplatesScreenshot(templateType = null) {
  const { updateTemplateItemState, templates } =
    React.useContext(TemplatesContext);
  ...
  React.useEffect(() => {
    screenshot.posts
      ...
      .forEach((post) =>
        updateTemplateItemState(post.id, { thumbnail: post.imageUrl }),
      );
  }, [screenshot.succeed]);
  ...
  React.useEffect(() => {
    screenshot.posts
      ...
      .forEach((post) =>
        updateTemplateItemState(post.id, { screenshot_url: null }),
      );
  }, [screenshot.failed]);
