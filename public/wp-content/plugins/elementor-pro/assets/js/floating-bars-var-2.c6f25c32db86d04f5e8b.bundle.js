        ...
        class FloatingBarsHandler extends _base.default {
          ...
          getDefaultElements() {
            ...
            return {
              ...
              closeButton: this.$element[0].querySelector(
                selectors.closeButton,
              ),
              pauseButton: this.$element[0].querySelector(
                selectors.pauseButton,
              ),
              playButton: this.$element[0].querySelector(selectors.playButton),
              headlineAll: this.$element[0].querySelectorAll(
                selectors.headline,
              ),
              headlines: this.$element[0].querySelector(selectors.headlines),
              headlinesInner: this.$element[0].querySelector(
                selectors.headlinesInner,
              ),
          ...
          isStickyTop() {
            const { isSticky, hasVerticalPositionTop } =
          ...
          isStickyBottom() {
            const { isSticky, hasVerticalPositionBottom } =
              this.getSettings("constants");
            return (
              this.elements.main.classList.contains(isSticky) &&
          ...
          hasStickyElements() {
            const stickyElements =
              document.querySelectorAll(".elementor-sticky");
          ...
          cloneScrollerContent() {
            const scrollerContent = Array.from(
              this.elements.headlinesInner.children,
            );
          ...
          cloneHeadlinesInner() {
            const headlinesInnerDuplicate =
              this.elements.headlinesInner.cloneNode(true);
            const overlay = this.elements.overlay;
            headlinesInnerDuplicate.classList.add(
              "e-floating-bars__headlines-inner--clone",
            );
            headlinesInnerDuplicate.setAttribute("aria-hidden", "true");
            this.elements.headlines.insertBefore(
              headlinesInnerDuplicate,
              overlay,
            );
          }
          handleBrowserScrollAnimation() {
            document.addEventListener("scroll", () => {
              ...
              const lastHeadline =
                this.elements.headlinesInner.lastElementChild;
              const isRTL = elementorFrontend.config.is_rtl;
              if (isRTL) {
                this.elements.headlinesInner.style.transform = `translateX(${scrollPosition}px)`;
                const lastHeadlineLeftPosition =
                  lastHeadline.lastElementChild.getBoundingClientRect().left;
                const headlinesLeftPosition =
                  this.elements.headlines.getBoundingClientRect().left;
                if (lastHeadlineLeftPosition >= headlinesLeftPosition) {
                  this.cloneScrollerContent();
                }
              } else {
                this.elements.headlinesInner.style.transform = `translateX(-${scrollPosition}px)`;
                const lastHeadlineRightPosition =
                  lastHeadline.lastElementChild.getBoundingClientRect().right;
                const headlinesRightPosition =
                  this.elements.headlines.getBoundingClientRect().right;
          ...
          handleStickyElements() {
            ...
            stickyElements.forEach((stickyElement) => {
              ...
              const isTop =
                "0px" === stickyElement.style.top || "top" === stickyPosition;
              const isBottom =
                "0px" === stickyElement.style.bottom ||
                "bottom" === stickyPosition;
              ...
              if (elementorFrontend.isEditMode()) {
                if (isTop) {
                  stickyElement.style.top = this.isStickyTop()
                    ? `${mainHeight}px`
                    : "0px";
                } else if (isBottom) {
                  stickyElement.style.bottom = this.isStickyBottom()
                    ? `${mainHeight}px`
                    : "0px";
                }
              }
            ...
            document
              ...
              .forEach((stickySpacer) => {
                const dataSettings = stickySpacer.getAttribute("data-settings");
                const stickyPosition = JSON.parse(dataSettings)?.sticky;
                const isTop =
                  "0px" === stickySpacer.style.top || "top" === stickyPosition;
                if (this.isStickyTop() && isTop) {
          ...
          initScrollingAnimation() {
            const { autoplay, animationTypeControl } =
              this.getSettings("constants");
            const animationType =
              this.getResponsiveSetting(animationTypeControl);
              ...
              if (this.isMobileDevice()) {
                this.elements.headlineAll.forEach((headline) => {
                  headline.addEventListener(
                    "click",
                    this.handleTickerClick.bind(this),
                  );
                });
