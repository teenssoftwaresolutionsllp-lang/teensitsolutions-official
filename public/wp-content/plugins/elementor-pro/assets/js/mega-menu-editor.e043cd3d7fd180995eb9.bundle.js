...
(self["webpackChunkelementor_pro"] =
      ...
      /***/ (module, __unused_webpack_exports, __webpack_require__) => {
        ...
        module.exports = ElementEditorModule.extend({
          ...
          maybeSetContentWidthValue() {
            const contentWidthControlView =
              this.getEditorControlView("content_width");
            const contentWidthControlValue =
              contentWidthControlView.getControlValue();
  ...
  \*******************************************************/
      /***/ (__unused_webpack_module, exports, __webpack_require__) => {
        ...
        var _nestedModule = _interopRequireDefault(
          __webpack_require__(
            /*! ./nested-module */ "../modules/mega-menu/assets/js/editor/nested-module.js",
          ),
        );
        var _editorModule = _interopRequireDefault(
          __webpack_require__(
            /*! ./editor-module */ "../modules/mega-menu/assets/js/editor/editor-module.js",
          ),
        );
  ...
  \***********************************************************/
      /***/ (__unused_webpack_module, exports) => {
        ...
        exports["default"] = void 0;
        class View
          extends $e.components.get("nested-elements").exports.NestedView
        {
          constructor() {
            super(...arguments);
          ...
          onAddChild(childView) {
            const widgetNumber =
                childView._parent.$el.find(".e-n-menu")[0]?.dataset
                  .widgetNumber || childView.model.attributes.widgetId,
          ...
          getChildViewContainer(containerView, childView) {
            ...
            if (
              ...
              return containerView.$el.find(`${childContainerSelector}`)[
                childView._index
              ];
            }
            if (customSelector) {
          ...
          renderOnChange(settings) {
            ...
            const renderResult = this.renderDataBindings(
              settings,
              this.dataBindings,
              widget,
            );
          ...
          renderDataBindings(settings, dataBindings) {
            ...
            const renderDataBinding = async (dataBinding) => {
              if (
                undefined !==
                settings.changed[dataBinding.dataset.bindingSetting]
              ) {
                dataBinding.el.innerHTML =
                  settings.changed[dataBinding.dataset.bindingSetting];
                return true;
              }
              ...
              if (
                ...
                return await this.getDynamicValue(
                  settings,
                  changedControl,
                  bindingSetting,
                  dataBinding,
            ...
            for (const dataBinding of dataBindings) {
              switch (dataBinding.dataset.bindingType) {
                case "repeater-item":
                  {
                    const repeater =
                      this.container.repeaters[
                        dataBinding.dataset.bindingRepeaterName
                      ];
                    if (!repeater) {
                      break;
                    }
                    const container = repeater.children.find(
                    ...
                    if (
                      ...
                      dataBindings.indexOf(dataBinding) + 1 ===
                      this.getRepeaterItemActiveIndex()
                    ) {
                      if (this.isItemLinkChild(widget)) {
                        return true;
                      }
                      changed = this.tryHandleDynamicCoverSettings(
                        dataBinding,
                        settings,
                      );
                    }
          ...
          async getDynamicValue(
            ...
            const data = await this.getDataFromCacheOrBackend(
              valueToParse,
              dynamicSettings,
            );
          ...
          getChangedDynamicControlKey(settings) {
            ...
            return this.isSettingChanged(settings, this.itemLink)
              ? this.itemLink
              : this.itemTitle;
          }
          tryFormatDynamicMegaMenuUrl(
            valueToParse,
            dataBinding,
            widget,
            changedControl,
            ...
            dataBinding.el = Array.from(widget)[0].querySelectorAll(
              ".e-n-menu-title-text",
            )[dataBinding.dataset.bindingIndex - 1];
          }
          ...
          getTitleWithAdvancedValues(settings, text) {
            const { attributes, _previousAttributes: previousAttributes } =
              settings;
            if (
              this.compareSettings(attributes, previousAttributes, "fallback")
            ) {
            ...
            const newBefore = this.getNewSettingsValue(
                attributes,
                previousAttributes,
                "before",
              ),
              newAfter = this.getNewSettingsValue(
                attributes,
                previousAttributes,
                "after",
              );
          ...
          compareSettings(attributes, previousAttributes, key) {
            return (
              ...
              previousAttributes[key] !== attributes[key]
            );
          }

          // TODO: delete in 3.27.0
          getNewSettingsValue(attributes, previousAttributes, key) {
            return previousAttributes[key] !== attributes[key]
              ? attributes[key] || ""
              : "";
          }
          ...
          getRepeaterItemActiveIndex() {
            return (
              this.getContainer().renderer.view.model.changed.editSettings
                .changed.activeItemIndex ||
              this.getContainer().renderer.view.model.changed.editSettings
                .attributes.activeItemIndex
            );
          }
          ...
          tryHandleDynamicCoverSettings(dataBinding, settings) {
            ...
            dataBinding.el.textContent = this.getTitleWithAdvancedValues(
              settings,
          ...
          isAdvancedDynamicSettings(attributes) {
            return (
              "before" in attributes &&
              "after" in attributes &&
              "fallback" in attributes
            );
          }
          ...
          isTitleOrLinkChanged(settings) {
            return (
              this.isSettingChanged(settings, this.itemTitle) ||
              this.isSettingChanged(settings, this.itemLink)
            );
          }
        }
