          ...
          extends elementorModules.frontend.handlers.Base
        {
          ...
          __construct() {
            ...
            elementorFrontend.hooks.addAction(
              ...
              this.loadResultElementsEvents.bind(this),
            );
          }
          getDefaultSettings() {
            return {
              selectors: {
                ...
                clearIcon:
                  ".e-search-input-wrapper > svg, .e-search-input-wrapper > i",
              },
            };
          }
          getDefaultElements() {
            ...
            return {
              resultsContainer: this.$element[0].querySelector(
                selectors.resultsContainer,
              ),
              searchField: this.$element[0].querySelector(
          ...
          setFocusableElements(elementContainer) {
            const focusableSelectors = (0,
            _focusableElementSelectors.focusableElementSelectors)();
            const focusableElements =
              elementContainer.querySelectorAll(focusableSelectors);
            this.focusableResultElements = Array.from(focusableElements).filter(
              (element) =>
                !element.disabled && !element.inert && element.tabIndex !== -1,
            );
          }
          ...
          bindEvents() {
            ...
            this.elements.searchField.addEventListener(
              "keydown",
              this.boundHandleKeyboardNavigation,
            );
            elementorFrontend.elements.window.document.addEventListener(
              "keydown",
          ...
          loadResultElementsEvents() {
            this.setFocusableElements(
              this.$element[0].querySelector(
                this.getSettings("selectors.resultsContainer"),
              ),
            );
          ...
          unbindEvents() {
            if (this.boundHandleKeyboardNavigation) {
              this.elements.searchField.removeEventListener(
                "keydown",
                this.boundHandleKeyboardNavigation,
              );
            }
            elementorFrontend.elements.window.document.removeEventListener(
              "keydown",
              this.boundHandleEscapeKey,
            );
            this.elements.searchField.removeEventListener(
          ...
          handleEscapeKey(event) {
            ...
            const activeElement =
              ...
              isSearchResultsFocused =
                this.elements.resultsContainer.contains(activeElement) || false;
          ...
          focusPreviousElement() {
            if (this.isSearchInputFocused()) {
              this.currentResultFocusedIndex =
                this.focusableResultElements.length - 1;
            } else {
              this.currentResultFocusedIndex--;
              this.checkFocusIndexBounds();
            }
          ...
          checkFocusIndexBounds() {
            if (
              this.currentResultFocusedIndex >=
              this.focusableResultElements.length
            ) {
          ...
          updateFocus() {
            if (-1 === this.currentResultFocusedIndex) {
              ...
              this.setFocusToElement(
                this.focusableResultElements[this.currentResultFocusedIndex],
              );
            }
          }
          closeResults() {
      ...
      /***/ (__unused_webpack_module, exports, __webpack_require__) => {
        ...
        class Search extends elementorModules.frontend.handlers.Base {
          ...
          __construct() {
            ...
            elementorFrontend.hooks.addAction(
              "search:results-displayed",
              this.hideOtherResults.bind(this),
            );
          }
          getDefaultSettings() {
            return {
          ...
          getDefaultElements() {
            ...
            return {
              ...
              searchWrapper: this.$element[0].querySelector(
                selectors.searchWrapper,
              ),
              searchField: this.$element[0].querySelector(
                selectors.searchField,
              ),
              submitButton: this.$element[0].querySelector(
          ...
          onInit() {
            ...
            this.toggleSearchResultsVisibility =
              this.toggleSearchResultsVisibility.bind(this);
          ...
          bindEvents() {
            ...
            this.elements.searchField.addEventListener(
              "keydown",
              this.onSearchFieldKeydown.bind(this),
            );
            this.elements.searchWidget.addEventListener(
              "click",
            ...
            ["focusin", "focusout"].forEach((eventType) => {
              this.elements.searchField.addEventListener(
                eventType,
                this.toggleWidgetFocusClass.bind(this),
              );
            });
            this.elements.clearIcon?.addEventListener(
              "click",
          ...
          onType(event) {
            ...
            if (
              ...
              this.elements.searchField.value.length >= minimumSearchLength
            ) {
              this.page_number = 1;
              this.renderLiveResults();
            }
          ...
          shouldShowLiveResults() {
            return (
              this.getElementSettings().live_results &&
              this.getElementSettings().template_id
            );
          }
          renderLiveResults() {
            ...
            return this.fetchUpdatedSearchWidgetMarkup()
              ...
              .finally(() => {
                const resultsElements = document.querySelectorAll(
                  `[data-id="${this.getID()}"] .e-loop-item`,
                );
          ...
          maybeHandleNoResults(resultsNode) {
            const isNoResultsMessage = !!resultsNode.querySelector(
              ".e-search-nothing-found-message",
            );
            this.elements.results.classList[
              isNoResultsMessage ? "add" : "remove"
            ]("no-results");
          ...
          createResultNode(responseData) {
            ...
            const searchResultsList = this.getSettings(
              "classes.searchResultsListWrapper",
            );
            const paginationNode = document.createElement("nav");
            const searchResultPagination = this.getSettings(
              "classes.searchResultsPagination",
            );
            const searchResultPaginationAriaLabel = "Pagination";
            paginationNode.setAttribute("class", searchResultPagination);
            paginationNode.setAttribute(
              "aria-label",
              searchResultPaginationAriaLabel,
            );
            ...
            const loopItems =
                resultContentNode.querySelectorAll(".e-loop-item"),
          ...
          updateAriaExpanded(expanded) {
            this.elements.searchField.setAttribute(
              "aria-expanded",
              expanded ? "true" : "false",
            );
          }
          updateAriaLabel(searchTerms) {
            if (searchTerms) {
              this.elements.resultsContainer.setAttribute(
                "aria-label",
          ...
          getClosestDataElementorId(element) {
            ...
            return closestParent
              ? closestParent.getAttribute("data-elementor-id")
              : 0;
          }
          debounce(callback) {
            var _this = this;
            ...
            return function () {
              ...
              _this.debounceTimeoutId = setTimeout(
          ...
          updateInputStyle() {
            ...
            const cssVariableNamesMap = {
              ...
              hiddenRoots = this.getAllDisplayNoneParents(
            ...
            for (const iconSlug of iconSlugs) {
              ...
              widgetStyle.setProperty(
                `--e-search-${cssVariableSlug}-absolute-width`,
                width + "px",
              );
              this.elements.searchField.classList[width ? "remove" : "add"](
                `no-${cssVariableSlug}`,
              );
            }
          ...
          changeClearIconVisibility(shouldHide) {
            this.elements.clearIcon?.classList[shouldHide ? "add" : "remove"](
              "hidden",
            );
          }
          toggleSearchResultsVisibility(event) {
            const selectors = this.getSettings("selectors"),
              ...
              isTargetPartOfResults =
                !!target?.closest(
                  `${widgetWrapper} ${selectors.resultsContainer}`,
                ) ||
                (target?.classList?.contains(selectors.resultsContainer) &&
                  !!target?.closest(widgetWrapper)),
              isSearchContainerClicked = !!target?.closest(
                `${widgetWrapper} ${selectors.searchWrapper}`,
              ),
              isSearchInputClicked = target?.classList?.contains(
                selectors.searchField.replace(".", ""),
              ),
              isSearchResultsPresent =
                this.elements.resultsContainer?.children?.length;
          ...
          getAllDisplayNoneParents(elementNode) {
            ...
            return this.getAllDisplayNoneParents(
          ...
          onElementChange(propertyName) {
            const propertyNameCallbackMap = {
              search_field_icon_label_size: () =>
                this.updateInputStyle(["searchIcon"]),
