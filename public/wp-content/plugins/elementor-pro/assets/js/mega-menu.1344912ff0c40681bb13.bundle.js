      ...
      /***/ (__unused_webpack_module, exports) => {
        ...
        class AnchorLinks {
          ...
          calculateRootMargin(element) {
            ...
            const isAnchorHeightLargerThanHalfViewport =
              anchorHeight > this.viewPortHeight / 2;
            const rootMarginBlockEnd = (-1 * this.viewPortHeight) / 2;
            const rootMarginBlockStart = isAnchorHeightLargerThanHalfViewport
              ? rootMarginBlockEnd
              : 0;
            ...
            return new IntersectionObserver((entries) => {
              entries.forEach((entry) => {
                $targetElement.toggleClass(
                  this.activeAnchorClass,
                  entry.isIntersecting,
                );
                $element.attr(
                  "aria-current",
                  entry.isIntersecting ? "location" : "",
                );
              });
            }, options);
          }
        ...
        class MegaMenu extends elementorModules.frontend.handlers.Base {
          ...
          getDefaultElements() {
            ...
            return {
              ...
              $dropdownMenuToggle: this.findElement(
              ...
              $tabContentsBeforeInterlacing: this.findElement(
                selectors.tabContentBeforeInterlacing,
              ),
            };
          }
          ...
          getTabIndex(tabTitleElement) {
            return tabTitleElement.getAttribute(
          ...
          setContentContainerAbsolutePosition($contentContainer) {
            const elementSettings = this.getElementSettings(),
              isFitToContent =
                "fit_to_content" === elementSettings.content_width;
            if ((0, _utils.isMenuInDropdownMode)(elementSettings)) {
              return;
            }
            if (isFitToContent) {
              const direction = elementorFrontend.config.is_rtl
                  ? "right"
                  : "left",
            ...
            if (
              this.shouldPositionContentAbove($contentContainer, headingsHeight)
            ) {
          ...
          getCenteredContainerOffset(contentContainerWidth, titleBoundingBox) {
            ...
            if (
              titleMiddleOffset + menuItemContentContainerHalfWidth >
              bodyWidth
            ) {
          ...
          getLeftDirectionContainerOffset(
            contentContainerWidth,
            titleBoundingBox,
          ) {
          ...
          getRtlLeftDirectionContainerOffset(
            ...
            const bodyWidth = elementorFrontend.elements.$body[0].clientWidth,
              titleLeftOffset = this.adjustForScrollbarIfNeeded(
                titleBoundingBox.left,
              );
          ...
          getLtrLeftDirectionContainerOffset(
            contentContainerWidth,
            titleBoundingBox,
          ) {
            let offset = this.adjustForScrollbarIfNeeded(titleBoundingBox.left);
            offset = this.adjustStartOffsetToViewport(
              offset,
              contentContainerWidth,
            );
            return offset;
          }
          getRightDirectionContainerOffset(
            contentContainerWidth,
            titleBoundingBox,
          ) {
            return elementorFrontend.config.is_rtl
          ...
          getRtlRightDirectionContainerOffset(
            ...
            let offset =
              bodyWidth -
              this.adjustForScrollbarIfNeeded(titleBoundingBox.right);
            offset = this.adjustStartOffsetToViewport(
              offset,
              contentContainerWidth,
            );
          ...
          getLtrRightDirectionContainerOffset(
            contentContainerWidth,
            titleBoundingBox,
          ) {
          ...
          getMenuContainerOffsetRtl(menuContainerBoundingBox) {
            const bodyWidth = elementorFrontend.elements.$body[0].clientWidth;
            let menuContainerOffset =
              bodyWidth - menuContainerBoundingBox.right;
            if (elementorFrontend.isEditMode()) {
          ...
          deactivateActiveTab() {
            const settings = this.getSettings(),
              ...
              $activeTitle =
                this.elements.$tabDropdowns.filter(activeTitleFilter),
              $activeContent =
                this.elements.$tabContents.filter(activeContentFilter);
          ...
          shouldPositionContentAbove($contentContainer) {
            ...
            const contentDimensions =
              $contentContainer[0].getBoundingClientRect();
            return (
              this.isContentShorterThanItsTopOffset(
                contentDimensions,
                offset,
          ...
          isContentTallerThanItsBottomOffset(contentDimensions) {
            return (
              window.innerHeight - contentDimensions.top <
          ...
          changeActiveTabByKeyboard(event, settings) {
            ...
            const $focusableElement = this.$element.find(
              ...
              isLinkElement =
                "a" === $focusableElement[0].tagName.toLowerCase(),
          ...
          onTabClick(event) {
            ...
            const hasNoDropdown =
              event?.currentTarget?.classList?.contains("link-only");
            ...
            const dropdownOpensWithHover = !this.isNeedToOpenOnClick(),
              blockMouseClickEvents =
                dropdownOpensWithHover && !this.isKeyboardNavigation;
            if (hasNoDropdown || blockMouseClickEvents) {
              return;
            }
            const selectors = this.getSettings("selectors"),
              getClickedMenuId = event?.target
                ?.closest(selectors.elementorWidgetWrapper)
            ...
            const clickedElement = event?.currentTarget,
              dropdownElement = clickedElement?.querySelector(
                selectors.tabDropdown,
              ),
          ...
          onMouseTitleEnter(event) {
            ...
            const titleStateAttribute =
                settings.ariaAttributes.titleStateAttribute,
              dropdownSelector = settings.selectors.tabDropdown,
              activeDropdownElement =
                currentTarget?.querySelector(dropdownSelector),
              isActiveTabTitle =
                "true" ===
                activeDropdownElement?.getAttribute(titleStateAttribute);
            if (isActiveTabTitle) {
              return;
            }
            const tabIndex =
              activeDropdownElement?.getAttribute("data-tab-index");
            this.changeActiveTab(tabIndex, true);
          }
          ...
          onClickOutsideDropdownMenu(event) {
            ...
            const settings = this.getSettings(),
              ...
              $activeContent =
                this.elements.$menuContent.find(activeContentFilter),
          ...
          isMenuContentHovered() {
            const settings = this.getSettings(),
              $widget = this.$element;
            return (
              $widget.find(`${settings.selectors.menuContent}:hover`).length > 0
            );
          }
          isCursorInBetweenMenuTitleAndContent(event) {
            ...
            const activeContent = currentElement
              ?.closest(selectors.menuItem)
              ?.querySelector(selectors.menuContent);
            const isMouseLeavingTabTitle = currentElement.classList?.contains(
              selectors.tabTitle.replace(".", ""),
            );
            const hasActiveTabTitle = activeContent?.classList?.contains(
              settings.classes.active,
            );
            if (!isMouseLeavingTabTitle || !hasActiveTabTitle) {
              return false;
            }
            const titleBoundingClientRect =
              currentElement.getBoundingClientRect();
            const contentBoundingClientRect =
              activeContent.getBoundingClientRect();
            const mouseY = event.clientY;
            const isTitleAboveContent =
              titleBoundingClientRect.bottom <= contentBoundingClientRect.top;
            return isTitleAboveContent
              ? mouseY >= titleBoundingClientRect.bottom &&
                  mouseY < contentBoundingClientRect.top
              : mouseY <= titleBoundingClientRect.top &&
                  mouseY > contentBoundingClientRect.bottom;
          }
          ...
          didCursorMoveSidewaysOrDown(event) {
            ...
            return (
              this.prevMouseY !== null && event?.clientY >= this.prevMouseY
            );
          }
          ...
          isHoveredDropdownMenu(isMouseLeavingTabContent, event) {
            ...
            if (
              isMouseLeavingTabContent &&
              this.didCursorMoveSidewaysOrDown(event)
            ) {
              return false;
            }
          ...
          onMouseLeave(event) {
            ...
            if (
              !this.isHoveredDropdownMenu(isMouseLeavingTabContent, event) &&
              !this.isCursorInBetweenMenuTitleAndContent(event)
            ) {
              this.deactivateActiveTab();
            }
          }
          onInit() {
            ...
            this.menuToggleVisibilityListener(
              this.elements.$dropdownMenuToggle,
            );
            this.setScrollPosition();
          ...
          activeContainerWidthListener($activeContainer) {
            ...
            this.observedContainer = new ResizeObserver((activeContainer) => {
              const currentWidth =
                activeContainer[0].borderBoxSize?.[0].inlineSize;
          ...
          changeScrollStatusAndDispatch(navigationWrapper, event) {
            (0, _flexHorizontalScroll.changeScrollStatus)(
              navigationWrapper,
              event,
            );
            elementorFrontend.elements.$window.trigger(
          ...
          updateIndexValues(view) {
            ...
            tabTitles.forEach((element, index) => {
              const newIndex = index + 1,
                ...
                updatedContainerID = updatedTabID.replace(
                  "e-n-menu-title-",
                  "e-n-menu-content-",
                ),
                updatedTabDropdownID = updatedTabID.replace(
                  "e-n-menu-title-",
              ...
              element
                ...
                ?.setAttribute("data-tab-index", newIndex);
              element
                .querySelector(settings.selectors.tabDropdown)
                ?.setAttribute("id", updatedTabDropdownID);
              element
                .querySelector(settings.selectors.tabDropdown)
              ...
              tabsContents[index]?.setAttribute(
                "aria-labelledby",
                updatedTabDropdownID,
              );
          ...
          maybeReplaceMenuItemTitleContent(
            elementParent,
            newElement,
            closestMenuItemTitle,
          ) {
          ...
          eCurrentClassHandler(actionName, closestMenuItemTitle, isCurrentUrl) {
            ...
            switch (actionName) {
              ...
              default:
                if (
                  closestMenuItemTitle.classList.contains(eCurrentClassName) &&
                  postUrl !== actionName
                ) {
                  closestMenuItemTitle.classList.remove(eCurrentClassName);
                }
          ...
          setTouchMode() {
            const widgetSelector =
              this.getSettings("selectors").widgetContainer;
