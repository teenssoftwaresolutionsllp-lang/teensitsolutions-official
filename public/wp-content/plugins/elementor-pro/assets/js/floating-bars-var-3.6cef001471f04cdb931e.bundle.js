        ...
        class FloatingBarsHandler extends _base.default {
          ...
          getDefaultElements() {
            ...
            return {
              ...
              couponButton: this.$element[0].querySelector(
                selectors.couponButton,
              ),
              couponCode: this.$element[0].querySelector(selectors.couponCode),
              codeTextGroup: this.$element[0].querySelector(
                selectors.codeTextGroup,
              ),
              successTextGroup: this.$element[0].querySelector(
                selectors.successTextGroup,
              ),
            };
          }
          ...
          bindEvents() {
            ...
            if (this.elements.couponButton) {
              this.elements.couponButton.addEventListener(
                "animationend",
            ...
            if (this.elements.couponButton) {
              this.elements.couponButton.addEventListener(
                "click",
                this.handleCouponButtonClick.bind(this),
              );
            }
          ...
          initEntranceAnimation() {
            ...
            const entranceAnimationClass = this.getResponsiveSetting(
              couponEntranceAnimation,
            );
            const entranceAnimationDelay =
              this.getResponsiveSetting(couponEntranceAnimationDelay) || 0;
          ...
          removeEntranceAnimationClasses() {
            ...
            const { animated, couponEntranceAnimation, visible } =
              this.getSettings("constants");
            const entranceAnimationClass = this.getResponsiveSetting(
              couponEntranceAnimation,
            );
          ...
          getDuration(duration) {
            ...
            const size =
              "" !== duration.size ? duration.size : DEFAULT_DURATION_SIZE;
            return isUnitSeconds ? size * 1000 : size;
          }
          handleCouponButtonClick(element) {
            const { successMessageDurationControl, isHidden } =
              this.getSettings("constants");
            const text = this.elements.couponCode.innerText;
            const successMessageDuration = this.getResponsiveSetting(
              successMessageDurationControl,
            );
            const duration = this.getDuration(successMessageDuration);
            const currentWidth =
              element.currentTarget.getBoundingClientRect().width;
            const currentHeight =
              element.currentTarget.getBoundingClientRect().height;
            this.elements.mainV3.style.setProperty(
              "--e-floating-bars-coupon-width",
              `${currentWidth}px`,
            );
            this.elements.mainV3.style.setProperty(
              "--e-floating-bars-coupon-height",
              `${currentHeight}px`,
            );
          ...
          initDefaultState() {
            ...
            if (
              this.elements.main &&
              !this.elements.couponButton.classList.contains(
                hasEntranceAnimation,
              ) &&
