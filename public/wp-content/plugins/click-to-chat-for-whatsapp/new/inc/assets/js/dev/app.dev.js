...
(function htCtcAppModule(window, document, ctcJq) {
  ...
  function initClickToChat() {
    ...
    var post_title =
      typeof document.title !== "undefined" ? document.title : "";
    ...
    function getStorageData() {
      ...
      if (localStorage.getItem("ht_ctc_storage")) {
        try {
          const ht_ctc_storage_raw = JSON.parse(
      ...
      show(
        ...
        if (ctcJq) {
          ...
          if (".ctc_opt_in" === target) {
            ctcJq(target).fadeOut(200).fadeIn(200).fadeOut(200).fadeIn(200);
            return;
          }

          ctcJq(target).show(duration || undefined);
          return;
        }
        ...
        els.forEach((el) => {
          ...
          if (classToAdd) {
            ...
            classToAdd.split(/\s+/).forEach((cls) => {
          ...
          if (classToRemove) {
            ...
            classToRemove.split(/\s+/).forEach((cls) => {
    ...
    function getValues() {
      ...
      if (Object.keys(ctc).length === 0 && chatData) {
        try {
          const settings = chatData?.getAttribute("data-settings") || "";
    ...
    function start() {
      ...
      document.dispatchEvent(
        new CustomEvent("ht_ctc_event_init", { detail: initDetail }),
      );
    ...
    function greetings() {
      ...
      if (ht_ctc_chat) {
        const greetingsBox = document.querySelector(
    ...
    function greetings_display() {
      ...
      if (greetingsBox) {
        ...
        if (greetingsTriggers.length > 0) {
          console.log(
            "greetings open triggers found: " + greetingsTriggers.length,
          );
    ...
    function createModalBackdrop() {
      ...
      if (!modal) {
        console.log(
          "No .ctc_greetings_modal found: skipping createModalBackdrop",
        );
        return;
      }
      ...
      if (notificationEl && ctc_getItem("n_badge") !== "stop") {
        ...
        if (badgeEl) {
          ...
          if (main && htCtcBadge) {
            ...
            if (top !== null) {
              htCtcBadge.style.top = top;
            }
            if (right !== null) {
              htCtcBadge.style.right = right;
            }
          }
        }
    ...
    function ht_ctc_things(chatElement) {
      ...
      var an_time = chatElement.classList.contains("ht_ctc_entry_animation")
        ? 1200
        : 120;
    ...
    function ht_ctc_chat_analytics(values) {
      ...
      function apply_variables(templateString) {
        ...
        var number =
          ctc.chat_number && "" !== ctc.chat_number
            ? ctc.chat_number
            : ctc.number;
        console.log(number);
        ...
        try {
          ...
          console.log(
            "window.apply_variables_value: " + window.apply_variables_value,
          );
      ...
      if (ctc.ga) {
        ...
        if (Array.isArray(ctc_values.g_an_params)) {
          ...
          ctc_values.g_an_params.forEach((paramKey) => {
            ...
            if (typeof paramKey !== "string" || !isSafeObjectKey(paramKey)) {
              return;
            }
        ...
        if (typeof dataLayer !== "undefined") {
          ...
          try {
            ...
            try {
              ...
              console.warn(
                "Error reading google_tag_data.tidr.destination",
                err,
              );
            }
                  ...
                  if (Array.isArray(container.destinations)) {
                    container.destinations.forEach((dest) => {
                      if (typeof dest === "string" && dest.startsWith("G-")) {
                        addMeasurementId(
                          dest,
                          "google_tag_data.container." + "destinations",
                        );
                      }
                    });
                  }
            ...
            console.log(
              "apply_variables placeholder replacement failed",
              error,
            );
          }
        }
        ...
        if (0 === gtag_count && "no" === is_ctc_add_gtag) {
          console.log(
            "gtag_count is 0 and gtag is not created by plugin. - " +
              "sending default event",
          );
          if (typeof gtag !== "undefined") {
            console.log(
              "calling gtag - default (no specifc send to parm. " +
                "g_event_name: " +
                g_event_name,
            );
            console.log("ga_params: ");
        ...
        if (ctc.gtm) {
          ...
          if (Array.isArray(ctc_values.gtm_params)) {
            ctc_values.gtm_params.forEach((gtmParamKey) => {
              if (
                typeof gtmParamKey !== "string" ||
                !isSafeObjectKey(gtmParamKey)
              ) {
                return;
              }
    ...
    function hook(number) {
      ...
      var hook_values =
        ctc.hook_v && typeof ctc.hook_v === "object" ? ctc.hook_v : {};
      ...
      if ("json" === ctc.webhook_format) {
        console.log(
          "main hook: json (sending as form-urlencoded to match jQuery/avoid preflight)",
        );

        // Convert to search params
        var params = new URLSearchParams();
        Object.keys(hook_values).forEach((key) => {
          ...
          params.append(
            key,
            typeof hookVal === "object" ? JSON.stringify(hookVal) : hookVal,
          );
        });
        ...
        data = new URLSearchParams(hook_values).toString();
      }
