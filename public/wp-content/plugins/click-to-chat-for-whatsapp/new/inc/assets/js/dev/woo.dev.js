...
(function htCtcWooModule($) {
  $(function handleWooReady() {
    ...
    function initializeCartLayout() {
      ...
      const singleCartButton = document.querySelector(
    ...
    function applyS1Styling(selector, sourceButton, multiple = false) {
      ...
      if (!sourceButton || !targetButtons.length || !targetButtons[0]) {
        return;
      }
      ...
      targetButtons.forEach((targetButton) => {
        ...
        const textColor = $(targetButton).css("color");
        const bgColor = $(targetButton).css("background-color");
    ...
    function applyS8Styling(selector, referenceButton) {
      ...
      if (!targetElements.length || !referenceButton) {
        return;
      }
