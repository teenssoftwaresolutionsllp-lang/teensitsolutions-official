(function ($) {
  ...
  var WidgetCustomGMHandler = function ($scope, $) {
    $scope.find(".map-render").each(function () {
      ...
      if (controls.style == "custom") {
        style = $.parseJSON(
          decodeURIComponent(atob($(this).attr("data-template"))),
        );
      } else {
      ...
      if (locations.markerlist != undefined) {
        locations.markerlist = $.parseJSON(
          decodeURIComponent(atob(locations.markerlist)),
        );
        if (Array.isArray(locations.markerlist)) {
          for (var i = 0; i < locations.markerlist.length; i++) {
            locations.markerdesc =
              '<div class="info-content"><h5>' +
  ...
  $(window).on("elementor/frontend/init", function () {
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/custom_google_map.default",
