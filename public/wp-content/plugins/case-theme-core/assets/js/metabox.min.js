(function ($) {
  ...
  SRFEMetabox.init = function () {
    ...
    var saveBtns = document.querySelectorAll(
      '#publishing-action .button[name="save"], #save-action .button[mame="save"], #addtag #submit, #edittag input[type="submit"]',
    );
  ...
  SRFEMetabox.getErrors = function (data) {
    ...
    for (var id in data.errors) {
      ...
      if ($field.length) {
        $field
          .parent()
          .append($('<div class="redux-th-error">').html(data.errors[id]));
      ...
      if (totalErrors > 0) {
        ...
        container.find(".redux-group-tab").each(function () {
          ...
          if (total > 0) {
            ...
            container
              .find('.redux-group-tab-link-a[data-key="' + sectionRel + '"]')
              .prepend('<span class="redux-menu-error">' + total + "</span>");
            container
              .find('.redux-group-tab-link-a[data-key="' + sectionRel + '"]')
              .addClass("hasError");
            var subParent = container
              .find('.redux-group-tab-link-a[data-key="' + sectionRel + '"]')
  ...
  SRFEMetabox.getWarnings = function (data) {
    ...
    for (var id in data.warnings) {
      ...
      if ($field.length) {
        $field
          .parent()
          .append($('<div class="redux-th-warning">').html(data.warnings[id]));
      ...
      if (totalErrors > 0) {
        ...
        container.find(".redux-group-tab").each(function () {
          ...
          if (total > 0) {
            ...
            container
              .find('.redux-group-tab-link-a[data-key="' + sectionRel + '"]')
              .prepend('<span class="redux-menu-warning">' + total + "</span>");
            container
              .find('.redux-group-tab-link-a[data-key="' + sectionRel + '"]')
              .addClass("hasError");
            var subParent = container
  ...
  SRFEMetabox.pageTemplates = function () {
    ...
    if (
      "undefined" == typeof data.page_templates ||
      $.isEmptyObject(data.page_templates)
    ) {
      return;
    }
    ...
    for (var boxid in data.page_templates) {
      ...
      if (
        -1 !== $.inArray($("#page_template").val(), data.page_templates[boxid])
      ) {
        if (!$box.is(":visible")) {
          $box.fadeIn(300);
  ...
  SRFEMetabox.postFormats = function (data) {
    ...
    if (
      "undefined" == typeof data.post_formats ||
      $.isEmptyObject(data.post_formats)
    ) {
      return;
    }
    ...
    for (var boxid in data.post_formats) {
      ...
      if (
        -1 !==
        $.inArray(
          $('input[name="post_format"]').val(),
          data.post_formats[boxid],
        )
      ) {
        if (!$box.is(":visible")) {
          $box.fadeIn(300);
  ...
  SRFEMetabox.formSubmit = function () {
    ...
    var submitBtn, $formFields;
    defaults = [];

    if ("edit-tags-php" == adminpage) {
      submitBtn = document.querySelector("form#addtag #submit");
      $formFields = $(
        ".redux-container input[name], .redux-container select[name], .redux-container textarea[name]",
      );
        ...
        submitBtn.addEventListener("click", function (e) {
          ...
          $formFields.each(function () {
            if ($(this).prev(".select2-container").length) {
              $(this)
                .val($(this).data("srfe-metabox-default-value"))
