(function ($) {
  ...
  var WidgetPostMasonryHandler = function ($scope, $) {
    ...
    $(".ct-grid").each(function () {
          ...
          setTimeout(function () {
            $.get(next_link, function () {})
              .done(function (data) {
                if (layout === "masonry") {
                  var items = $(data).find(
                    "#" + html_id + " .ct-grid-masonry > .grid-item",
                  );
                  var time = 0.4;
                  items.each(function () {
                    $(this).addClass("ct-animated");
                    $(this)
                      .find(".grid-item-inner")
                      .css("animation-duration", time + "s");
                    time = time + 0.15;
                  });
                  $(items).imagesLoaded(function () {
                    $("#" + html_id)
                      .children(".ct-grid-masonry")
                      .append(items);
                    sep_grid_refresh();
      ...
      if (
        typeof _pagination_variable !== "undefined" ||
        _pagination_variable.length > 0
      ) {
        $("#" + html_id + " .ct-grid-pagination").on(
          "click",
          ".page-numbers",
          function (e) {
            e.preventDefault();
              ...
              $.get(_p_link, function () {})
                .done(function (data) {
                  if (masonry[html_id] === "masonry") {
                    var _contents = $(data).find(
                      "#" + html_id + " .ct-grid-inner",
                    );
                    var _pagination = $(data).find(
                      "#" + html_id + " .ct-grid-pagination",
                    );
                    var items = $(data).find(
                      "#" + html_id + " .ct-grid-masonry > .grid-item",
                    );
                    ...
                    $(items).imagesLoaded(function () {
                      $("#" + html_id)
                        ...
                        .html(_contents.html());
                      $("#" + html_id)
                        .find(".ct-grid-pagination")
                        .html(_pagination.html());
                      document
                        .getElementById(html_id)
                        .scrollIntoView({
                          behavior: "smooth",
                          block: "start",
                          inline: "nearest",
